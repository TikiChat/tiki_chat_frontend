...
import 'package:tikichat_app/Presentation/View/Components/tk_cubit_view.dart';
...
class _AgreeScreenState extends State<AgreeScreen> {
  ...
  Widget build(BuildContext context) {
    return TkLayout(
      child: TkCubitView<AuthViewModel, Terms>(
        createCubit: (context) => AuthViewModel()..getAgreeTerms(),
        successBuilder: (context, state) {
          Terms terms = state.items[0];
          if (agreeList.isEmpty) {
            agreeList = terms.terms.map((e) => false).toList();
          }
          return contentWidget(terms: terms);
        },
      ),
    );
  }

  Widget contentWidget({required Terms terms}) {
    return Column(
